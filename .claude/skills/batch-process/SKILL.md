...
Always use `--no-plan`. Default max-iterations: 30. If the user specifies a number of items to process and the checklist was generated with that same limit, use that number as max-iterations too (e.g. user asks for 50 items → `--limit 50` → max-iterations 50). The user can also override max-iterations explicitly. Optional: `--stop-on-manual-test` if user requests it.
...
```bash
unset CLAUDECODE && ./ralph_json.sh <checklist> <instruction> <max-iterations> --no-plan
```
...
```bash
unset CLAUDECODE && ./ralph_json.sh <checklist> <instruction> <max-iterations> --no-plan 2>&1 | tee <task-dir>/ralph.log
```
