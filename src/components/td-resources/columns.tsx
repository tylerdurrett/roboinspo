...
/** Create columns with hub context for proper link generation */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function createColumns(hubSlug: Hub): ColumnDef<ResourceWithRelations, any>[] {
  const hubConfig = getHubConfig(hubSlug)
  const basePath = hubConfig.basePath
  ...
  return [
    ...
    columnHelper.accessor('pricingModel', {
      ...
      cell: ({ row }) => {
        const pricing = row.original.pricingModel
        return (
    ...
    columnHelper.accessor('skillLevels', {
      ...
      cell: ({ row }) => {
        const levels = row.original.skillLevels ?? []
        if (levels.length === 0)
